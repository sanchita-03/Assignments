using PolicyManagement.Constants;
using PolicyManagement.Model;
using PolicyManagement.Repository;

namespace PolicyManagement
{
    internal class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Welcome To NeoInsurance");
            IPolicyRepository policyRepository = new PolicyRepository();
            int choice,id;
            while (true)
            {
                Console.WriteLine("*****************************************************************************");
                Console.WriteLine("Choose The Options :");
                Console.WriteLine("1.Add a New Policy\n2.View All Policies\n3.Search Policy by ID\n4.Update Policy Details");
                Console.WriteLine("5.Delete a Policy\n6.View Active Policies\n7.Exit");
                Console.WriteLine("Enter Your Choice");
                Console.WriteLine("*****************************************************************************");
                choice = Convert.ToInt32(Console.ReadLine());
                switch (choice)
                {
                    case 1:
                        Console.WriteLine("Enter Policy Holder name");
                        string name = Console.ReadLine();
                        Console.Write("Enter Policy Type (1.Life, 2.Health, 3.Vehicle, 4.Property): ");
                        PolicyType type = (PolicyType)Enum.Parse(typeof(PolicyType), Console.ReadLine(), true);
                        Console.WriteLine("Your Start Date (yyyy-MM-dd) is : ");
                        DateTime startDate = DateTime.Now.Date;
                        Console.WriteLine(startDate);
                        Console.WriteLine("Enter End Date (yyyy-MM-dd): ");
                        DateTime endDate = DateTime.Parse(Console.ReadLine());
                        int addpolicy=policyRepository.AddPolicy(new Policy(){PolicyHolderName=name,PolicyType=type,StartDate=startDate,EndDate=endDate});
                        if (addpolicy > 0)
                        {
                            Console.WriteLine("Policy Added Successfuly");

                        }
                        Console.WriteLine("*****************************************************************************");
                        break;
                    case 2:
                        Console.WriteLine("*****************************************************************************");
                        Console.WriteLine("Details Of All Policies");
                        List<Policy> getAllPolicies = policyRepository.ViewAllPolicies();
                        foreach (Policy item in getAllPolicies)
                        {
                            Console.WriteLine(item);
                        }
                        Console.WriteLine("*****************************************************************************");
                        break;
                    case 3:
                        Console.WriteLine("*****************************************************************************");
                        Console.WriteLine("Enter Policy Id You Want To Search");
                        id = Convert.ToInt32(Console.ReadLine());
                        Policy searchbyid=policyRepository.SearchPolicyById(id);
                        if (searchbyid != null)
                        {
                            Console.WriteLine($"Details of Policy {id} :");
                            Console.WriteLine(searchbyid);
                        }
                        
                        Console.WriteLine("*****************************************************************************");
                        break;
                    case 4:
                        Console.WriteLine("*****************************************************************************");
                        Console.WriteLine("Enter Policy Id for which you want to update Details");
                        id = Convert.ToInt32(Console.ReadLine());
                        int updatepolicy = policyRepository.UpdatePolicyDetails(id);
                        if (updatepolicy > 0)
                        {
                            Console.WriteLine($"Policy {id} updated Successfuly");
                        }
                        Console.WriteLine("*****************************************************************************");
                        break;
                    case 5:
                        Console.WriteLine("*****************************************************************************");
                        Console.WriteLine("Enter Policy Id You Want To Delete");
                        id = Convert.ToInt32(Console.ReadLine());
                        int deletepolicy=policyRepository.DeletePolicy(id);
                        if (deletepolicy > 0)
                        {
                            Console.WriteLine($"Policy {id} Deleted Successfuly");
                        }
                        else
                        {
                            Console.WriteLine("Failed to delete");
                        }
                        Console.WriteLine("*****************************************************************************");
                        break;
                    case 6:
                        Console.WriteLine("*****************************************************************************");
                        Console.WriteLine("Details Of Active Policies:");
                        List<Policy>activePolicies=policyRepository.ViewActivePolicies();
                        
                        foreach (var item in activePolicies)
                        {
                                Console.WriteLine(item);
                        }
                       
                        Console.WriteLine("*****************************************************************************");
                        break;
                    case 7:
                        Console.WriteLine("*****************************************************************************");
                        Console.WriteLine("Exit");
                        Console.WriteLine("Thank you For Visiting");
                        Console.WriteLine("*****************************************************************************");
                        return;
                    default:
                        Console.WriteLine("*****************************************************************************");
                        Console.WriteLine("invalid Input Provided Please give valid input(1-7)");
                        Console.WriteLine("*****************************************************************************");
                        break;
                }
            }

        }
    }
}
